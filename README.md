...
The web service starts polling on boot and repeats every `POLL_INTERVAL_MINUTES`, defaulting to `30`. Scheduled polls only run on the configured `POLL_DAYS`, which defaults to Saturday and Sunday in IST.
